<template>
  <div class="orders-container">
    <div class="header-row">
      ...
      <h1 class="title">Активные заказы</h1> </div>

    <div class="top-bar">
      <div class="button-group-right">
        <button
            class="btn approve-selected"
            @click="extendSelectedOrders" :disabled="!selectedIds.length"
        >Продлить аренду</button> <button
          class="btn reject-selected"
          @click="confirmReturnSelectedOrders" :disabled="!selectedIds.length"
...
export default {
  ...
  methods: {
    ...
    toggleSort(field) {
      ...
      this.loadActiveOrders(); // Вызываем метод загрузки активных заказов
    },
    ...
    async extendSelectedOrders() {
      ...
      try {
        ...
        console.error('Ошибка при продлении аренды заказов:', err);
    ...
    async confirmReturnSelectedOrders() {
      ...
      try {
        ...
        console.error('Ошибка при подтверждении возврата заказов:', err);
        this.errorMessage = 'Ошибка подключения к серверу при подтверждении возврата заказов';
      }
    },
    goBack() {
...
.btn.reject-selected:hover { /* Это теперь кнопка "Подтвердить возврат" */
  background-color: #22c55e; /* Пример: более насыщенный зеленый цвет */
  color: white;
}
